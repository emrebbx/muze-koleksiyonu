...
function v37NasilOynanirAc() {
  ...
  k.innerHTML = `
    <div class="v37Panel v37NasilPanel">
...
function v37TurGuncelle() {
  ...
  if (!el) return;
  el.textContent = `SIRA: ${V37.oyuncuAdlari[aktifOyuncu] || `${aktifOyuncu}. OYUNCU`}`;
  el.dataset.oyuncu = String(aktifOyuncu);
}

function v37SkorGuncelle() {
  const a = document.getElementById('v37Skor1');
  const b = document.getElementById('v37Skor2');
  if (a) a.textContent = `${V37.oyuncuAdlari[1]}  •  +${V37.puanlar[1]} Koleksiyon`;
  if (b) b.textContent = `${V37.oyuncuAdlari[2]}  •  +${V37.puanlar[2]} Koleksiyon`;
}

setInterval(() => { if (v37OyunAktif) { v37TurGuncelle(); v37SkorGuncelle(); } }, 250);
...
new MutationObserver(() => setTimeout(v37KoleksiyonlariKontrolEt, 80)).observe(oynanisEkrani, { childList: true, subtree: true });
...
function v37CopTarayiciAc() {
  ...
  const sayac = kat.querySelector('.v37CopSayac');
  function ciz() {
    const k = copDeste[idx];
    resim.src = k.dosya;
    baslik.textContent = k.tip === 'eser' ? 'ÇÖPTEKİ ESER' : 'ÇÖPTEKİ ÖZEL KART';
    sayac.textContent = `${idx + 1} / ${copDeste.length}  •  En yeni kart sağ tarafta`;
  }
...
/* =====================================================
   V39 — 23.08.2026 SON TEST DÜZELTMELERİ
   - oyuncu isimleri tüm kritik mesajlarda
   - ortak oturum havuzu alt başlığı
   - Nasıl Oynanır stabil açılış
   - sanatçı kart sırası sabit
   - Gizli Sandık rastgele/gizli
   - Müzede Deprem kaldırıldı
   - Aktivist Saldırısı eklendi
===================================================== */

/* -----------------------------------------------------
   MÜZEDE DEPREM -> AKTİVİST SALDIRISI
----------------------------------------------------- */
(function v39OzelKartHavuzunuGuncelle() {
  const depremIndex = ozelKartlar.findIndex(k => k.id === 'muzede-deprem');
  const aktivist = { id:'aktivist-saldirisi', tip:'ozel', dosya:'images/aktivist-saldirisi.png' };
  if (depremIndex !== -1) ozelKartlar.splice(depremIndex, 1, aktivist);
  else if (!ozelKartlar.some(k => k.id === aktivist.id)) ozelKartlar.push(aktivist);
  if (!benzersizGorseller.includes(aktivist.dosya)) benzersizGorseller.push(aktivist.dosya);
})();

function v39OyuncuAdi(oyuncu) {
  return (typeof V37 !== 'undefined' && V37.oyuncuAdlari?.[oyuncu]) || `${oyuncu}. OYUNCU`;
}

/* -----------------------------------------------------
   SEÇİM / TUR MESAJLARI: NUMARA YERİNE İSİM
----------------------------------------------------- */
const v39EskiSecimYazisiGoster = secimYazisiGoster;
secimYazisiGoster = function(metin) {
  let m = String(metin ?? '');
  m = m.replaceAll('1. OYUNCU', v39OyuncuAdi(1));
  m = m.replaceAll('2. OYUNCU', v39OyuncuAdi(2));
  v39EskiSecimYazisiGoster(m);
  const y = document.getElementById('secimYazisi');
  if (!y) return;
  const turMesaji = /ORTAK DESTEDEN KART ÇEK|ÖZEL KART KULLAN|EKSTRA KART|SON EKSTRA KART|KARTINI KENDİN ÇEK/.test(m);
  y.classList.toggle('v39TurMesaji', turMesaji);
};

/* -----------------------------------------------------
   ZAR EKRANI: OYUNCU İSİMLERİ
----------------------------------------------------- */
const v39EskiBaslangicZarSisteminiBaslat = baslangicZarSisteminiBaslat;
baslangicZarSisteminiBaslat = function() {
  v39EskiBaslangicZarSisteminiBaslat();
  requestAnimationFrame(() => {
    const adlar = document.querySelectorAll('#baslangicZarKatmani .zarOyuncuAdi');
    if (adlar[0]) adlar[0].textContent = v39OyuncuAdi(1);
    if (adlar[1]) adlar[1].textContent = v39OyuncuAdi(2);
  });
};

const v39EskiBaslangicZarlariniKarsilastir = baslangicZarlariniKarsilastir;
baslangicZarlariniKarsilastir = function() {
  const esitlik = oyuncu1ZarSonucu === oyuncu2ZarSonucu;
  v39EskiBaslangicZarlariniKarsilastir();
  if (!esitlik) {
    const sonuc = document.getElementById('zarGenelSonuc');
    if (sonuc) sonuc.textContent = `${v39OyuncuAdi(aktifOyuncu)} BAŞLIYOR!`;
  }
};

/* -----------------------------------------------------
   12 SANATÇI KARTINI TAM SABİT TUT
----------------------------------------------------- */
(function v39SanatciSirasiSabit() {
  const s = document.getElementById('kapaliKartSirasi');
  if (!s) return;
  const sabitle = () => {
    if (s.classList.contains('goster')) s.classList.add('v39Sabit');
    else s.classList.remove('v39Sabit');
  };
  new MutationObserver(sabitle).observe(s, { attributes:true, attributeFilter:['class'] });
  ['pointerdown','pointerup','pointercancel'].forEach(t => s.addEventListener(t, (e) => {
    if (e.target.closest?.('.kapaliSanatciKarti')) {
...
/* -----------------------------------------------------
   NASIL OYNANIR: TÜM ESKİ LISTENER'LARI TEMİZLE
   MENÜ MÜZİĞİNE DOKUNMA
----------------------------------------------------- */
(function v39NasilOynaniriStabillestir() {
  ...
  yeni.addEventListener('click', (e) => {
    e.preventDefault();
    e.stopImmediatePropagation();
    if (typeof v37EskiButonSesiCal === 'function') v37EskiButonSesiCal();
    else butonSesiCal();
    v37NasilOynanirAc();
  }, true);
})();
...
/* -----------------------------------------------------
   ÖZEL KART BAŞLIĞI: AYRI ALT SATIR
----------------------------------------------------- */
const v39EskiOzelKartSeciminiBaslat = ozelKartSeciminiBaslat;
ozelKartSeciminiBaslat = function() {
  v39EskiOzelKartSeciminiBaslat();
  const baslik = document.getElementById('ozelKartBaslik');
  if (baslik) baslik.textContent = 'ÖZEL KART SEÇİMİ';
  let alt = document.getElementById('ortakOturumHavuzu');
  if (!alt) {
    alt = document.createElement('div');
    alt.id = 'ortakOturumHavuzu';
    baslik?.insertAdjacentElement('afterend', alt);
  }
  alt.textContent = 'ORTAK OTURUM HAVUZU';
};

/* -----------------------------------------------------
   GİZLİ SANDIK: DIŞARIDAKİ KARTLAR GÖRÜNMEZ,
   RASTGELE 1 TANE ÇEKİLİR
----------------------------------------------------- */
gizliSandikKullan = function(kartBilgisi, kartElementi, katman) {
  if (!artanOzelKartlar.length) {
    ozelKartKararlariniGoster(kartBilgisi, kartElementi, katman);
    return;
  }

  ozelKartiOturumDisinaCikar(kartBilgisi, kartElementi, katman, () => {
    document.getElementById('v39GizliSandikKatmani')?.remove();
    const secim = document.createElement('div');
    secim.id = 'v39GizliSandikKatmani';
    secim.innerHTML = `
      <div class="v39Panel">
        <h2>GİZLİ SANDIK</h2>
        <p>Oyun dışında kalan özel kartların kimliği gizlidir.</p>
        <p>Rastgele 1 özel kart çekilecek.</p>
        <button type="button">RASTGELE KART ÇEK</button>
      </div>`;
    document.body.appendChild(secim);

    secim.querySelector('button').addEventListener('click', () => {
      const index = Math.floor(Math.random() * artanOzelKartlar.length);
      const gelen = artanOzelKartlar.splice(index, 1)[0];
      if (!gelen) { secim.remove(); turuBitir(); return; }
      ...
      secim.remove();
      secimYazisiGoster('GİZLİ SANDIK — RASTGELE ÖZEL KART ÇEKİLDİ');
...
/* -----------------------------------------------------
   ...
   - rakibin tamamlanmamış koleksiyonu
   - Müze Güvenliği altında değil
----------------------------------------------------- */
function aktivistSaldirisiHedefleri() {
  return rakibinTamamlanmamisUygunEserleri();
}

const v39EskiOzelKartKullanilabilirMi = ozelKartKullanilabilirMi;
ozelKartKullanilabilirMi = function(kartBilgisi) {
  if (kartBilgisi?.id === 'aktivist-saldirisi') return aktivistSaldirisiHedefleri().length > 0;
  return v39EskiOzelKartKullanilabilirMi(kartBilgisi);
};

const v39EskiOzelKartUyariMetni = ozelKartUyariMetni;
ozelKartUyariMetni = function(kartBilgisi) {
  if (kartBilgisi?.id === 'aktivist-saldirisi') {
    return aktivistSaldirisiHedefleri().length ? '' : 'Saldırılabilecek uygun eser yok.';
  }
  return v39EskiOzelKartUyariMetni(kartBilgisi);
};

function aktivistSaldirisiKullan(kartBilgisi, kartElementi, katman) {
  const hedefler = aktivistSaldirisiHedefleri();
  if (!hedefler.length) {
    ozelKartKararlariniGoster(kartBilgisi, kartElementi, katman);
    return;
  }

  /* Kullanılan özel kart önce çöpe gider, sonra hedef seçilir. */
  kullanilanOzelKartiCopeAt(kartBilgisi, kartElementi, katman, () => {
    const guncelHedefler = aktivistSaldirisiHedefleri();
    if (!guncelHedefler.length) { turuBitir(); return; }
    ...
    rakipEserHedefSecimiBaslat(
      ...
      (secilenler) => {
        const hedef = secilenler[0];
        if (!hedef) { turuBitir(); return; }
        ...
        ozelKartZarEkraniAc(
          ...
          (zarSonucu) => {
            if (zarSonucu <= 4) {
              secimYazisiGoster('AKTİVİST SALDIRISI BAŞARISIZ — ESER KURTULDU');
              turuBitir();
              return;
            }

            secimYazisiGoster('AKTİVİST SALDIRISI BAŞARILI — ESER ÇÖPE GİDİYOR');
            masadakiEseriCopeGonder(hedef, () => turuBitir());
          }
        );
      }
    );
...
/* En son tanımlı yönlendiriciyi sar, diğer tüm kartları aynen koru. */
const v39EskiOzelKartiKullan = ozelKartiKullan;
ozelKartiKullan = function(kartBilgisi, kartElementi, katman) {
  if (kartBilgisi?.id === 'aktivist-saldirisi') {
    aktivistSaldirisiKullan(kartBilgisi, kartElementi, katman);
    return;
  }
  return v39EskiOzelKartiKullan(kartBilgisi, kartElementi, katman);
};

/* Deprem eski fonksiyonları dosyada kalabilir ama havuzda artık kart yoktur. */
console.log('V39 özel kart havuzu:', ozelKartlar.map(k => k.id));
