  ...
  function acikArtirmaSonucuKontrolEt() {
    ...
    if (
      ...
      genelSonuc.innerHTML =
        "EŞİTLİK<br>TEKRAR ZAR ATILACAK";
...
function v42CopDesteSirasiZorla() {
  ...
  /*
    DOM sırasını da veri sırasıyla aynı hale getir.
    Böylece z-index dışı tarayıcı davranışında bile
    son kart son child olur.
  */
  copDeste.forEach((kartBilgisi) => {
    const adaylar =
      Array.from(document.querySelectorAll(".coptekiKart"))
        .filter((el) => el.dataset.id === kartBilgisi.id);

    const el = adaylar[adaylar.length - 1];

    if (el) {
      oynanisEkrani.appendChild(el);
    }
  });
}
...
baslangicZarEkraniniKapat = function() {
  ...
  return v44EskiBaslangicZarEkraniniKapat();
};
...
oyunMuzikButonu?.addEventListener("click", async (event) => {
  ...
  if (oyunMuzigiKapatildi) {
    await muzikKapat(oyunMuzik, 280);
    oyunMuzik.pause();
    return;
  }
...
arkeolojikKaziKullan = function(kartBilgisi, kartElementi, katman) {
  ...
  kullanilanOzelKartiCopeAt(kartBilgisi, kartElementi, katman, () => {
    ...
    const altKart = document.createElement("img");
    altKart.src = "images/kart-arkasi.png";
    altKart.className = "v60KaziAltKart";

    Object.assign(altKart.style, {
      position: "fixed",
      left: `${desteRect.left + 3}px`,
      top: `${desteRect.top + 8}px`,
      width: `${desteRect.width}px`,
      height: `${desteRect.height}px`,
      objectFit: "fill",
      zIndex: "9290",
      pointerEvents: "none",
    ...
    /* Önce yığının en altından dışarı sıyrılır. */
    gsap.to(altKart, {
      x: -desteRect.width * .72,
      y: desteRect.height * .10,
      rotation: -7,
      zIndex: 9400,
      duration: .62,
      ease: "power2.out",
      onComplete: () => {
        /* Kart kendi dikey ekseninde dönerken gerçek yüzü açılır. */
        gsap.to(altKart, {
          scaleX: 0,
          duration: .20,
          ease: "power1.in",
          onComplete: () => {
            ...
            gsap.to(altKart, {
              ...
              onComplete: () => {
                setTimeout(() => {
                  /* Normal kart büyütme sistemi kaynak olarak bu kartı kullanır. */
                  altKart.classList.add("ortakDesteGorselKarti");
                  cekilenKartiBuyukGoster(cekilen);
                  altKart.remove();
                  bekleyenOzelKart = null;
                  ozelKartEfektiAktif = false;
...
console.log("V56 hazır: Arkeolojik Kazı kartı destenin altından sıyrılıp normal boyuta büyüyor.");
