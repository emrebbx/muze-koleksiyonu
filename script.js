...
async function v37GercekOyunuBaslat() {
  ...
  v98YeniOturumKalintilariniTemizle();
  ilkOyunDurumunuHazirla();
  V37.puanlar = { 1: 0, 2: 0 };
...
function v37NasilOynanirAc() {
  ...
  function kartGalerisiniGoster() {
    ...
    /* Rehberdeki kartlar oyun dizilerinin gelişigüzel başlangıç
       sırasından bağımsız, oyuncunun belirlediği katalog sırasıyla
       gösterilir. Kopyaları sıraladığımız için oyun destesinin akışı
       ve karıştırma davranışı değişmez. */
    const sanatciKatalogSirasi = [
      'leonardo', 'van-gogh', 'durer', 'millet', 'monet', 'munch',
      'vermeer', 'velazquez', 'osman-hamdi', 'rembrandt', 'cezanne',
      'mondrian'
    ];
    const ozelKartKatalogSirasi = [
      'dedektif-sorusturmasi', 'restorasyon', 'muzede-kaza',
      'eser-calindi', 'eser-sahteydi', 'arti-iki-kart',
      'arkeolojik-kazi', 'acik-artirma', 'depo-kilidi', 'gece-nobeti',
      'gizli-sandik', 'muze-muduru', 'muze-guvenligi',
      'ozel-kart-takasi', 'yeni-sergi', 'aktivist-saldirisi',
      'fransa-takimi', 'hollanda-takimi', 'klasikler-takimi',
      'dunya-takimi'
    ];
    const siraHaritasi = liste =>
      new Map(liste.map((id, index) => [id, index]));
    const sanatciSirasi = siraHaritasi(sanatciKatalogSirasi);
    const ozelKartSirasi = siraHaritasi(ozelKartKatalogSirasi);
    const katalogSirala = (kartlar, siraBul) =>
      [...kartlar].sort((a, b) => siraBul(a) - siraBul(b));

    const siraliSanatcilar = katalogSirala(
      sanatcilar,
      kart => sanatciSirasi.get(kart.id) ?? Number.MAX_SAFE_INTEGER
    );
    const siraliEserler = katalogSirala(
      eserKartlari,
      kart =>
        (sanatciSirasi.get(kart.sanatci) ?? Number.MAX_SAFE_INTEGER) * 10 +
        (Number(kart.eserSirasi) || 0)
    );
    const siraliOzelKartlar = katalogSirala(
      ozelKartlar,
      kart => ozelKartSirasi.get(kart.id) ?? Number.MAX_SAFE_INTEGER
    );

    bolumEkle('SANATÇI KARTLARI', siraliSanatcilar);
    bolumEkle('ESER KARTLARI', siraliEserler);
    bolumEkle('ÖZEL KARTLAR', siraliOzelKartlar);
    icerik.appendChild(geriButonuOlustur(anaSecimiGoster));
    panel.scrollTop = 0;
  }
...
function botV1NormalTur() {
  ...
  if (
    ...
    hedefSecimAktif ||
    onizlemeAcik
    || window.v96OtomatikArtiIkiCekisiPlanli
  ) {
...
function v55SonrakiOyunaGec() {
  ...
  const kaliciEserIdleri = new Set(
    Array.from(document.querySelectorAll(
      ".yerlesenEserKarti, .depodakiEserKarti"
    )).map(kart => kart.dataset.id).filter(Boolean)
  );
  const yeniDesteEserIdleri = new Set();
  const yeniDeste = copDeste
    .filter(kart => {
      if (kart.tip !== "eser") return true;
      if (kaliciEserIdleri.has(kart.id)) return false;
      if (yeniDesteEserIdleri.has(kart.id)) return false;
      yeniDesteEserIdleri.add(kart.id);
      return true;
    })
    .map(kart => ({ ...kart }));
...
v51DokuzEserKontrolu = function() {
  v99TekrarlananEserKayitlariniTemizle();
...
artiIkiKartKullan = function(kartBilgisi, kartElementi, katman) {
  ...
  const akisiBaslat = () => {
    ...
    /* Otomatik izleme modunda aktif oyuncu insan koltuğu da olabilir.
       +2'nin ilk ekstra çekişini DOM'daki deste görselini arayan genel
       bot turuna bırakmak bazen akışı beklemede tutuyordu. İlk çekişi
       burada tam bir kez yap; ikinci çekiş turuBitir kuyruğunda kalır. */
    if (window.v78OtomatikOyun) {
      window.v96OtomatikArtiIkiCekisiPlanli = true;
      kartCekmeKilidi = true;
      ...
      setTimeout(() => {
        ...
        window.v96OtomatikArtiIkiCekisiPlanli = false;
        kartCekmeKilidi = false;
        BOT_V1.kilit = false;
...
/* =====================================================
   V94 — YUVADAN AÇILAN ÖZEL KARTIN KULLAN / KAPAT KARARI
===================================================== */

function v94BotEldekiOzelKartKarari() {
  if (
    typeof BOT_V1 === "undefined" ||
    !BOT_V1.aktif ||
    aktifOyuncu !== BOT_V1.oyuncu
  ...
  const katman = document.getElementById("eldekiOzelKartKatmani");
  if (!botV1GorunurMu(katman)) return false;

  const butonlar = Array.from(katman.querySelectorAll("button"));
  const kullan = butonlar.find(buton =>
    String(buton.textContent || "")
      .toLocaleUpperCase("tr-TR")
      .trim() === "KULLAN"
  );
  const kapat = butonlar.find(buton =>
    String(buton.textContent || "")
      .toLocaleUpperCase("tr-TR")
      .trim() === "KAPAT"
  );

  const karar = kullan && !kullan.disabled && botV1GorunurMu(kullan)
    ? kullan
    : kapat;

  if (!karar || karar.disabled || !botV1GorunurMu(karar)) {
    return true;
  }
...
console.log("V94 hazır: bot yuvadaki özel kartı kullanır veya pencereyi kapatır.");

/* =====================================================
   V98 — YENİ OTURUMDA ESKİ KAZANAN EKRANI KORUMASI
===================================================== */

function v98YeniOturumKalintilariniTemizle() {
  /* DOM temizlenirken skor gözlemcisi eski dokuz eseri tekrar oyun sonu
     olarak yorumlamasın. Temizlik bitince yeni oturum sayaçları açılır. */
  V51_OTURUM.oyunBitiyor = true;
  V55_DOKUZLA_OTURUM_BITTI = true;

  const kalintilar = document.querySelectorAll([
    "#v51OyunSonu",
    "#secimYazisi",
    ".yerlesenSanatciKarti",
    ".yerlesenEserKarti",
    ...
    "#baslangicZarKatmani",
    "#sanatciSecimBanner",
    "#v41SanatciSecimBanner"
  ].join(","));

  kalintilar.forEach(element => {
    gsap.killTweensOf(element);
    element.remove();
  });
...
console.log("V98 hazır: yeni oturum eski masa ve kazanan ekranı kalıntıları olmadan başlar.");

/* =====================================================
   V99 — HER ESER KARTI OYUNDA YALNIZ BİR KEZ BULUNUR
===================================================== */

function v99TekrarlananEserKayitlariniTemizle() {
  const koleksiyondakiIdler = new Set(
    Array.from(document.querySelectorAll(".yerlesenEserKarti"))
      .map(kart => kart.dataset.id)
      .filter(Boolean)
  );
  const gorulenDepoIdleri = new Set();

  document.querySelectorAll(".depodakiEserKarti").forEach(depoKarti => {
    const kartId = depoKarti.dataset.id;
    const sahip = Number(depoKarti.dataset.oyuncu);
    const koleksiyondaVar = koleksiyondakiIdler.has(kartId);
    const depodaIkinciKopya = gorulenDepoIdleri.has(kartId);

    if (!koleksiyondaVar && !depodaIkinciKopya) {
      gorulenDepoIdleri.add(kartId);
      return;
    }

    gsap.killTweensOf(depoKarti);
    depoKarti.remove();

    const depo = oyuncununDeposu(sahip);
    const index = depo.findIndex(kart =>
      kart.id === kartId &&
      (!depoKarti.dataset.yuvaId || kart.yuvaId === depoKarti.dataset.yuvaId)
    );
    if (index !== -1) depo.splice(index, 1);

    console.warn("Tekrarlanan eser depo kaydı temizlendi:", kartId);
  });
}

console.log("V99 hazır: eser kartları deste, koleksiyon ve depolarda benzersiz tutulur.");
