...
function v80YedekKartGorseli(kartBilgisi) {

  const baslik = v80KartBasliginiBul(kartBilgisi)
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;");

  const tur = kartBilgisi?.tip === "eser"
    ? "ESER KARTI"
    : "ÖZEL KART";

  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="700" height="1050" viewBox="0 0 700 1050">
    <rect width="700" height="1050" rx="34" fill="#efe0b8" stroke="#2b190d" stroke-width="28"/>
    <rect x="45" y="45" width="610" height="960" rx="22" fill="none" stroke="#bd8e38" stroke-width="8"/>
    <text x="350" y="330" text-anchor="middle" fill="#5a3515" font-family="Georgia,serif" font-size="52" font-weight="bold">${tur}</text>
    <foreignObject x="80" y="390" width="540" height="300">
      <div xmlns="http://www.w3.org/1999/xhtml" style="height:300px;display:flex;align-items:center;justify-content:center;text-align:center;color:#2e1a0d;font:bold 58px Georgia,serif;line-height:1.18">${baslik}</div>
    </foreignObject>
    <text x="350" y="820" text-anchor="middle" fill="#77562e" font-family="Georgia,serif" font-size="31">Kart görseli yüklenemedi</text>
    <text x="350" y="870" text-anchor="middle" fill="#77562e" font-family="Georgia,serif" font-size="27">Kart bilgisi korunarak oyuna devam ediliyor</text>
  </svg>`;
...
function v80KartGorseliniHazirla(kartBilgisi, tamamlandi) {

  document.getElementById("v80KartYukleniyor")?.remove();
  const yuklemeKimligi = ++v80KartYuklemeKimligi;
  const yukleniyor = document.createElement("div");
  yukleniyor.id = "v80KartYukleniyor";
  yukleniyor.innerHTML = `<strong>KART AÇILIYOR…</strong><span>${v80KartBasliginiBul(kartBilgisi)}</span>`;
  document.body.appendChild(yukleniyor);
  document.body.classList.add("v84KartYukleniyorAcik");

  let tamam = false;
  const kontrol = new Image();

  const bitir = (gosterimDosyasi) => {
    if (tamam) return;
    tamam = true;
    clearTimeout(kesinZamanAsimi);
    yukleniyor.remove();
    if (yuklemeKimligi !== v80KartYuklemeKimligi) return;
    document.body.classList.remove("v84KartYukleniyorAcik");
    tamamlandi(gosterimDosyasi);
  };

  /*
    Bu zamanlayıcı yükleme olaylarından tamamen bağımsızdır. Tarayıcı ne
    onload ne de onerror üretse bile otomatik oyun burada takılı kalamaz.
  */
  const kesinZamanAsimi = setTimeout(() => {
    bitir(v80YedekKartGorseli(kartBilgisi));
  }, 2500);

  kontrol.onload = () => bitir(kontrol.src);
  kontrol.onerror = () => bitir(v80YedekKartGorseli(kartBilgisi));
  kontrol.src = kartBilgisi.dosya;
}
...
function cekilenKartiBuyukGoster(
  ...
  if (!gorselHazir) {
    v80KartGorseliniHazirla(
      kartBilgisi,
      (gosterimDosyasi) => {
        cekilenKartiBuyukGoster(
          { ...kartBilgisi, _gosterimDosyasi: gosterimDosyasi },
          true
        );
      }
    );
  ...
  kart.src =
    kartBilgisi._gosterimDosyasi ||
    kartBilgisi.dosya;


  // Geçici/yedek görsel adresi oyun durumuna veya kayıtlı karta taşınmasın.
  delete kartBilgisi._gosterimDosyasi;
